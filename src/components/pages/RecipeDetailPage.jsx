  ...
  return (
    ...
    <div className="max-w-6xl mx-auto">
        ...
        <div className="relative h-96 md:h-[500px] overflow-hidden rounded-t-2xl">
            ...
            <img
                src={recipe?.imageUrl || "/api/placeholder/800/600"}
            ...
            <div
                className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1.5 flex items-center space-x-2">
                <ApperIcon
    ...
    <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-12 shadow-recipe-card mb-6">
                <div className="flex border-b border-cream-200">
                    {tabs.map(tab => <button
                        ...
                        className={`flex-1 flex items-center justify-center space-x-2 px-6 py-4 text-sm font-medium transition-all duration-200 ${activeTab === tab.id ? "text-terracotta-500 border-b-2 border-terracotta-500 bg-terracotta-50/50" : "text-charcoal-500 hover:text-terracotta-500 hover:bg-cream-100"}`}>
                ...
                <div className="p-6">
                    {/* Ingredients Tab */}
                    {activeTab === "ingredients" && <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-charcoal-500">Ingredients
                                                      </h3>
                            <div className="flex items-center space-x-3">
                                <label className="text-sm font-medium text-charcoal-500">Servings:
                                                            </label>
                    ...
                    {/* Instructions Tab */}
                    {activeTab === "instructions" && <div className="space-y-6">
                        <h3 className="text-lg font-semibold text-charcoal-500">Instructions
                                                </h3>
                        {recipe.instructions && recipe.instructions.length > 0 ? <div className="space-y-4">
                            {recipe.instructions.map((instruction, index) => <motion.div
                                key={index}
                                initial={{
                                    opacity: 0,
                                    x: -20
                                ...
                                <div
                                    className="flex-shrink-0 w-8 h-8 bg-terracotta-50 text-terracotta-500 rounded-full flex items-center justify-center text-sm font-medium">
                                    {index + 1}
                                </div>
                    ...
                    {activeTab === "notes" && <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-charcoal-500">Recipe Notes
                                                </h3>
                        {recipe.notes ? <div className="bg-saffron-50 border border-saffron-200 rounded-lg p-4">
                            <p className="text-charcoal-500 leading-relaxed">
                                {recipe.notes}
                            </p>
                        </div> : <div className="text-center py-8 text-gray-500">
                        ...
                        {recipe.sourceUrl && <div className="pt-4 border-t border-cream-200">
                            <h4 className="text-sm font-medium text-charcoal-500 mb-2">Original Source
                                                        </h4>
                            <a
        ...
        <div className="space-y-6">
            {/* Recipe Tags */}
            {recipe.tags && recipe.tags.length > 0 && <div className="bg-white rounded-12 shadow-recipe-card p-6">
                <h3 className="text-lg font-semibold text-charcoal-500 mb-4">Tags
                                    </h3>
                <div className="flex flex-wrap gap-2">
                    {recipe.tags.map((tag, index) => <span key={index} className="ingredient-pill">#{tag}
                    </span>)}
                </div>
            </div>}
            {/* Quick Actions */}
            <div className="bg-white rounded-12 shadow-recipe-card p-6">
                <h3 className="text-lg font-semibold text-charcoal-500 mb-4">Quick Actions
                                  </h3>
            ...
            {/* Recipe Stats */}
            <div className="bg-white rounded-12 shadow-recipe-card p-6">
                <h3 className="text-lg font-semibold text-charcoal-500 mb-4">Recipe Stats
                                  </h3>
                <div className="space-y-3">
